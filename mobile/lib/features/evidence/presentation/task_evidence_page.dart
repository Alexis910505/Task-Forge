...
import '../evidence_providers.dart';

class TaskEvidencePage extends ConsumerStatefulWidget {
  const TaskEvidencePage({super.key, required this.taskId});

  final String taskId;

  @override
  ConsumerState<TaskEvidencePage> createState() => _TaskEvidencePageState();
}

class _TaskEvidencePageState extends ConsumerState<TaskEvidencePage> {
  ...
  Future<void> _refreshServer() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _serverAttachments = data is List ? List<dynamic>.from(data) : const [];
        });
      }
    } catch (e) {
      if (mounted) {
        setState(() => _serverError = e is DioException ? (e.message ?? '$e') : '$e');
      }
    }
  }
      ...
      if (path == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('No se pudo obtener la foto (permiso o cámara cancelada).')),
          );
        }
        return;
      }
      final captured = DateTime.now().toUtc();
      final (lat, lng) = await EvidenceCaptureService.optionalLocation(_includeGps);
      final result = await ref.read(evidenceUploadWorkerProvider).enqueue(
  ...
  Widget build(BuildContext context) {
    ...
    final pendingAsync = ref.watch(evidenceQueueForTaskProvider(widget.taskId));

    ref.listen(evidenceQueueForTaskProvider(widget.taskId), (prev, next) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          Text('Tarea', style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
          SelectableText(widget.taskId, style: theme.textTheme.bodySmall),
          const SizedBox(height: 16),
          SwitchListTile(
            title: const Text('Incluir geolocalización'),
            subtitle: const Text('Opcional: se adjunta lat/lng al registrar la evidencia.'),
            value: _includeGps,
                ...
                children: rows.map((r) {
                  return Card(
                    child: ListTile(
                      ...
                      title: Text('${r.evidenceKind} · intentos ${r.attempts}'),
                      subtitle: Text(
                        '${r.capturedAtIso}\n${r.localPath}',
                        maxLines: 3,
                        overflow: TextOverflow.ellipsis,
                      ),
                      trailing: r.lastError != null
                          ? Icon(Icons.error_outline, color: theme.colorScheme.error)
                          : null,
                      onTap: r.lastError != null
                          ? () {
                              ScaffoldMessenger.of(context).showSnackBar(
        ...
        if (error != null)
          ...
          Container(
            ...
            child: GridView.builder(
              ...
              itemBuilder: (context, i) => _EvidenceThumbTile(
                item: items[i],
...
class _EvidenceThumbTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final badgeBg = isBefore
        ? scheme.secondaryContainer
        : isAfter
            ? scheme.tertiaryContainer
            : scheme.surfaceContainerHighest;
    final badgeFg = isBefore
        ? scheme.onSecondaryContainer
        : isAfter
            ? scheme.onTertiaryContainer
            : scheme.onSurfaceVariant;
    final badgeLabel = isBefore
        ? 'ANTES'
        : isAfter
            ? 'DESPUÉS'
                ...
                child: Stack(
                  ...
                  children: [
                    ColoredBox(
                      color: scheme.surfaceContainerHigh,
                      child: url.isEmpty
                          ? Icon(Icons.image_outlined, color: scheme.onSurfaceVariant, size: 22)
                          : Image.network(
                              url,
                              fit: BoxFit.cover,
                      ...
                      Positioned(
                        ...
                        child: DecoratedBox(
                          ...
                          child: const Padding(
                            ...
                            child: Icon(Icons.location_on, size: 11, color: Colors.white),
                          ),
                        ),
                      ),
                  ],
